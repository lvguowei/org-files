...
******** Exercise 4.4
         ...
         *or*: The expressions are evaluated from left to right.
         ...
         #+BEGIN_SRC scheme
           ...
           (define (rest-predicates predicates) (cdr predicates))
           (define (no-predicate predicates) (null? predicates))
           (define (eval-and-predicates predicates env)
             (cond ((no-predicate predicates)
                    true)
                   ((no-predicate (rest-predicates predicates))
                    (eval (first-predicate predicates) env))
                   ((true? (eval (first-predicate predicates) env))
                    (eval-and-predicates (rest-predicates predicates) env))
                   (else
                    false)))
...
***** Z Flag
      ...
      After any comparison (CPX, CPY or CMP), the Z flag wlil be
